...
def task4():
    ...
    по странам (структура данных в примере). Необходимо написать код, который рассчитает среднюю температуру за
    период в Цельсиях(!) для каждой страны.
    ...
    for c in countries_temperature:
        print(f'{c[0]}  -   {(sum(c[1]) - (32 * len(c[1]))) / 1.8 / len(c[1]):.1f}')
...
def task5():
    ...
    my_dict = {}
    # создадим словарь ключ, значение, чтобы посчитать кол-во просмотров по пользователю
    for x in stream:
        if my_dict.get(x.split(',')[1]):
            my_dict[x.split(',')[1]] = my_dict.get(x.split(',')[1]) + [int(x.split(',')[2])]
        else:
            my_dict[x.split(',')[1]] = [int(x.split(',')[2])]

    # высчитываем соотношение по кол-ву всех просмотров на кол-во уникальных пользователей
    print(sum(sum(my_dict.values(), [])) / len(my_dict))
...
def task6():
    ...
    1 2 15 200
    """
    in_num_list = [int(x) for x in input().split()]
    print(' '.join(map(str, sorted(set(int(x) for x in in_num_list if in_num_list.count(x) > 1)))))
